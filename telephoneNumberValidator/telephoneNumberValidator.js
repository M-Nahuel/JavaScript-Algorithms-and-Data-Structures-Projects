...
function telephoneCheck(str) {
    const regex = [
      ...
      /^(1?\s?\({1}[0-9]{3}\){1}\s?[0-9]{3}\-?\s?[0-9]{4})$/,
      /^(1?\s?[0-9]{3}\-?\s?[0-9]{3}\-?\s?[0-9]{4})$/]
      return regex.some(elem=>elem.test(str));
}

check.addEventListener('click', () => {
  const valid = telephoneCheck(num.value) ?
  ''
  : 'not ';
  result.textContent = `The number provided is ${valid}a valid US phone number.`;
});